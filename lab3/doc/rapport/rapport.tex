...
\subsection{Entraînement du réseau de neurones}
    ...
    Pour cela, il est souvent utile d'afficher la précision d'entraînement et la précision de test.
    Ce que nous entendons ici par la précision, c'est en fait l'\textit{accuracy} qui est donnée par $\frac{TP+TN}{TP+FP+TN+FN}$.
    ...
    Deuxièmement, elle est facile à obtenir, car elle est calculée directement à la sortie du réseau de neurones et sert ensuite pour le calcul de la fonction de coût.
    \\
...
Par ailleurs, il y a des situations où le résultat est erroné, mais pour lesquels on peut comprendre l'origine de l'erreur :\\
...
Enfin, il y a également des faux positifs non explicables, bien que ceux-ci soient très peu nombreux :\\
