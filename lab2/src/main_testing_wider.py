...
if __name__ == '__main__':
    ...
    try:
        ...
        print("    int1 : numéro du dossier d'images à utiliser, entre 1 et 62")
        print("    int2 : nombre d'images à utiliser pour les tests de détection")
        exit(1)

    face_cascade = cv2.CascadeClassifier("../haarcascades/haarcascade_frontalface_default.xml")
    # get the image information
    fd = open("../dataset/WIDER_train/wider_face_split/wider_face_train_bbx_gt.txt")
    img_info_vec = []
    for k in range(nb_images_testing):
        img_info_vec.append(parse_file.get_img_info_wider(fd, file_number_used))
    fd.close()
